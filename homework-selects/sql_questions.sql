...
FROM employees e1
         INNER JOIN employees e2 ON e2.last_name = 'Taylor' AND e1.department_id = e2.department_id;
--10. the department name and number of employees in each of the department.
SELECT department_name, COUNT(employee_id) AS NymberOfEmployees
FROM employees
         INNER JOIN departments USING (department_id)
GROUP BY department_name;
--11. the name of the department, average salary and number of employees working in that department who got commission.
SELECT department_name, AVG(salary), COUNT(employee_id) AS NymberOfEmployees
FROM employees
         INNER JOIN departments USING (department_id)
...
SELECT *
FROM employees
WHERE department_id IN (SELECT department_id FROM employees WHERE employee_id NOT BETWEEN 100 AND 200);
...
SELECT employee_id, (first_name || ' ' || last_name) employee, job_id
FROM employees
WHERE salary > (SELECT AVG(salary) FROM employees);
